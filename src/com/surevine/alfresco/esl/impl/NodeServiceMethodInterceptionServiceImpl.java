...
/**
 * Concrete implementation of NodeServiceMethodInterceptionService providing methods to link NodeRefs to the MethodInvocations
 * that contain them.
 * 
 ...
 *
 */
public class NodeServiceMethodInterceptionServiceImpl implements
		NodeServiceMethodInterceptionService {
    ...
    /**
	 * Get all the noderefs used in the given method call that we should protect with an ESC by parsing the Spring
	 * config.  Any NodeRef we protect with an ACL we'll also protect with an ESC.
	 * @param config
	 * @param mi
	 * @return
	 */
	...
	/**
	 * For a given Spring ConfigAttribute, parse the text in the XML file and work out which parameter in the 
	 * protected method it is that the security definition is trying to protect.  Note that as a private method,
	 * a minimal of checking is performed around the input parameters.
	 * @param attrVal Value of a Config Attribute
	 * @return index into the method parameters at which the config indicates a NodeRef can be found.  
	 * We should run an ESC against that NodeRef
	 */
