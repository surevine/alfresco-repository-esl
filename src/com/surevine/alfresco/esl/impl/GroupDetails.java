 ...
 * </ul>
 * 
 * Permission authorities are a comma-seperated list of user - department pairs. Eg,
 * 
 * <pre>
 * usera-deptone,userb-dept2
 * </pre>
 * 
 * . Whitespace and order are irrelevant here.
 * 
 * Keys may be defined in any case, in any order. The only optional key is "deprecated", which defaults to "false". A key may only be defined once for a given constraint. Whitespace is allowed, and will be stripped (whitespaces _within_ values will not be stripped). Anything is allowed in a value, except for =. System name has a bit more logic to it - after trimming, it may only consist of letters and numbers and will be converted to upper case. "Deprecated" may take only the values "true" or "false", in any case
 * 
...
public class GroupDetails {
    ...
    /**
     * The only field to have a default value (false) and hence the only optional key in the specification string. If true, then the value exists at the backend and behaves normally, but user interfaces should handle the deprecation somehow, such as by not displaying the field, or by marking it as deprecated
     */
     ...
     * 
     * @param permissionAuthoritiesSpecification
     *            comma seperated list of name & department pairs where the name and department are hyphen seperated. eg. <code>Firstname Surname - department1,name surname-department2,name surname-department3</code> Whitespace is normalised and both a name and department must be specified for each permission authority
     */
