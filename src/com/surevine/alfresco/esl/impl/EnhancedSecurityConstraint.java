...
/**
 * Subclass of RMLOVConstraint with identical RLS behavior, but which incorporates a number of display-related metadata designed to
 * inform and operate the Enhanced Security user interface
 * @author simonw
 *
 */
public class EnhancedSecurityConstraint extends RMListOfValuesConstraint {
	 ...
	 */
	private enum DisplayPriority {LOW("Low"), HIGH("High"); private String _pri; DisplayPriority(String pri) { _pri=pri;} public String toString() { return _pri;}};
	
	/**
	 * How much priority (in practice, how far up or down the interface) should be given to display this constraint in a UI.  Note that
	 * the internal representation of this field is via the DisplayPriority enum which ensures correctness, but the property is exposed as a String
	 * via the public interface of this class to enable easier integration with Spring and to create a simpler interface.
	 */
	private DisplayPriority _displayPriority;
	
	/**
	 * If true, then the backend should hide from users values for this constraint which aren't actually assigned to the users.  A value of 'false' is
	 * only valid when MatchLogic=true
	 */
	...
	public void setEnhancedSecurityLocator(EnhancedSecurityConstraintLocator locator)
	{
		_locator=locator;
	}
	 ...
	 * @param systemName System-facing name (should be [A-Z0-9]+) to register
	 * @param humanName Human-facing name to register
	 * @throws EnhancedSecurityException If either the system name or the human name has already been used in this constraint
	 */
	...
	/**
	 * @param description A human-readable description of what the constraint is for - maybe a sentance or two in length.  Must be set to a non-null, non-all-whitespace value
	 */
	...
	/**
	 * @param displayPriority Hint to the UI as to how much important to attach to this constraint.  Must be set to either "low" or "high".  Case insensitive.
	 */
	...
	/**
	 * @param filterDisplay If true, then indicate to clients of this class that they should hide values in this constraint that users don't have from those users.  Note
	 * that this class doesn't, and indeed can't, enforce that, it's up to clients of the class
	 */
	...
	/**
	 * Although we're a subclass of RMLOVConstraint, we have a richer way of setting the allowed values, setGroupDetailsSpecification, so although we need to support
	 * the method through the subclass relationship, we throw an exception if anyone actually calls it
	 * @throws EnhancedSecurityException all the time
	 */
	 ...
	 * @param groupDetailsSpecification A list of String values taken from the model XML file.  See GroupDetails for the exact syntax.  Must be non-null and
	 * non-empty
	 */
    ...
    /**
     * Regenerate a specification for the current state of the constraint.  This will usually return the configuration that was used to create the constraint,
     * minus some normalisation of case and whitespace, but will reflect any changes that have occurred since then
     * @return
     */
    ...
    /**
     * Retrieve details on the values (aka groups) managed by this constraint.  A richer version of getAllowedValues(), although
     * getAllowedValues() is still allowed and supported
     * @return
     */
    ...
    /**
     * Best hook I (simonw) could find for adding some extra validation.  The first time we use the constraint to evaluate something, run the validate() method
     * then pass behaviour to superclass
     */
    @Override
    ...
    /**
     * Does some validation of the constraint as a whole (individual values within the constraint are validated at construction time but we can't do that for the constraint as a whole
     * as the framework uses setter-injection) then pokes a flag to indicate that validation is successful, to prevent us from continuously validating something that never really changes
     * @throws EnhancedSecurityException If validation fails for whatever reason
     */
    ...
    /**
     * Included for completeness - mark the constraint as not yet validated.  If programmatic changes are made to the constraint that might affect it's validity, this method
     * should then be called.  At the time of writing, nothing needs to do this
     */
	...
	public String toString()
	{
        ...
        sb.append("EnhancedSecurityConstraint")
